<div class="row">
	<?php
		...
		//Đếm tổng sp để chia cho giới hạn sp được xem từ đó có đc số trang
		$c_sql = "select count(*) as num_rows from sanpham where MaLoai = $idsp";
		$c_rs = load($c_sql);
		...
		$sql = "select * from sanpham where MaLoai = $idsp limit $offset, $limit";
		$rs = load($sql);
	...
	<!-- `````````````````````````PHẦN THỂ HIỆN SẢN PHẨM THEO LOẠI``````````````````````````````````````` -->
		...
		<div class="thumbnail" style="height: 400px">
			...
			<div class="caption">
				...
				<div class="container-fluid">
					...
					<!-- </div>  đặt if ở đây thì bị nhẩy sản phẩm, méo hiểu sao luôn-->
...
<tfood class="col-md-2 col-md-offset-5">
	<ul class="pager">	
		<?php if ($Lui > 0) : ?>
		<li>
			<a href="?idsp=<?= $idsp ?>&page=<?= $Lui ?>" role="button">
		...
		<li>
			<a href="?idsp=<?= $idsp ?>&page=<?= $Tien ?>" role="button">
				Tiếp
